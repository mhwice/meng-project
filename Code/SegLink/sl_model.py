...
from SegLink.ssd_model import ssd512_body, hybrid_body, hybrid_one, ssd512_trunc_body, ssd512v2_trunc_body
from SegLink.ssd_layers import Normalize
...
def SL512_trunc(input_shape=(512, 512, 3), softmax=True):
    # SSD body
    x = input_tensor = Input(shape=input_shape)
    source_layers, layer_five_output = ssd512_trunc_body(x)
    ...
    output_tensor = multibox_head(source_layers, num_priors, normalizations, softmax)
    model = Model(input_tensor, output_tensor)
    minimodel = Model(input_tensor, layer_five_output)
    
    # parameters for prior boxes
    model.image_size = input_shape[:2]
    model.source_layers = source_layers
    
    return model, minimodel


def SL512v2_trunc(input_shape=(512, 512, 3), softmax=True):
    # SSD body
    x = input_tensor = Input(shape=input_shape)
    source_layers, layer_five_output = ssd512v2_trunc_body(x)
...
def SL512v2(input_shape=(512, 512, 3), softmax=True):
    """SegLink512 architecture.
