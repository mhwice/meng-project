...
# ORIGINAL, UN-MODIFIED CRNN ARCHITECTURE
...
# IGNORE
def CRNNv3(input_shape, num_classes, prediction_only=False, gru=True):
    """CRNN architecture.
...
# DSC CRNN
def CRNNv2(input_shape, num_classes, prediction_only=False, gru=True):
    """CRNN architecture.
    ...
    x = DepthwiseConv2D(kernel_size=3, padding='same', depth_multiplier=1, strides=(1,1), use_bias=False, name='conv4_11111')(x)
    x = BatchNormalization(name='batchnorm111')(x)
    x = Activation('relu', name='act111')(x)
    x = Conv2D(filters=512, kernel_size=1, strides=1, padding='same', use_bias=False, name='conv4_22222')(x)
    x = BatchNormalization(name='batchnorm222')(x)
    x = Activation('relu', name='act222')(x)

    # x = Conv2D(512, (3, 3), strides=(1, 1), activation=act, padding='same', name='conv5_1')(x)
    # x = BatchNormalization(name='batchnorm2')(x)

    x = DepthwiseConv2D(kernel_size=3, padding='same', depth_multiplier=1, strides=(1,1), use_bias=False, name='conv5_11111')(x)
    x = BatchNormalization(name='batchnorm333')(x)
    x = Activation('relu', name='act333')(x)
    x = Conv2D(filters=512, kernel_size=1, strides=1, padding='same', use_bias=False, name='conv5_22222')(x)
    x = BatchNormalization(name='batchnorm444')(x)
    x = Activation('relu', name='act444')(x)
